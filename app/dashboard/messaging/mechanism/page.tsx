...
const QUESTIONS = [
    "What are the most common approaches or “best practices” your industry teaches?",
    "What’s missing or flawed in those approaches?",
    "What do your ideal clients actually want instead?",
    "Identify 3–5 experts or competitors in your space. Study what they claim makes them different — and spot patterns.",
    "What patterns do you notice across your competitors?",
    "What is the core frustration or “market lie” your mechanism solves?",
    "What truth or belief does your method stand on?",
    "What’s the one shift or key difference in how you help clients get results?",
    "How does your process remove friction or make success easier/faster?",
    "What is the step-by-step process or system you use to deliver your results?",
    "What name can you give this process so it’s clear, branded, and memorable?",
    "What category or lane do you now own because of this mechanism?",
    "What belief, promise, or guarantee can you confidently make that others can’t?",
    "What identity does this mechanism give your clients?"
]

export default function UniqueMechanismPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch('/api/messaging/mechanism')
    ...
    const saveProgress = async (newAnswers: string[], newReport: string | null = null) => {
        try {
            await fetch('/api/messaging/mechanism', {
                method: 'POST',
    ...
    const generateReport = async () => {
        setGenerating(true)
        try {
            const response = await fetch('/api/generate/mechanism', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        } catch (err) { alert("Error generating blueprint.") }
        finally { setGenerating(false) }
    }

    const downloadReport = () => {
        if (!result) return
        const header = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>Unique Mechanism Blueprint</title></head><body>";
        ...
        element.download = "Unique_Mechanism_Blueprint.doc"
                ...
                <DashboardLayout>
                    <div style={{ marginBottom: '1.5rem' }}>
                        <h2 style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            ...
                            Refining Your Unique Mechanism Blueprint
                        </h2>
                        <p style={{ color: 'var(--muted)', marginTop: '0.5rem' }}>
                            Use the AI assistant to perfect your proprietary method.
                        </p>
                    </div>
                    <SplitPanelLayout
                        leftPanel={
            ...
            <DashboardLayout>
                <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                        <h2>Unique Mechanism Blueprint</h2>
        ...
        <DashboardLayout>
            <div style={{ maxWidth: '700px', margin: '0 auto' }}>
                <div style={{ marginBottom: '2.5rem', textAlign: 'center' }}>
                    <span style={{ color: 'var(--primary)', fontWeight: '700', fontSize: '0.75rem', textTransform: 'uppercase' }}>Step 4</span>
                    <h2 style={{ fontSize: '2rem', marginTop: '0.5rem' }}>Unique Mechanism</h2>
                ...
                <div className="glass-card" style={{ padding: '2.5rem' }}>
                    ...
                    <textarea className="input-field" style={{ minHeight: '150px' }} value={answers[step]} onChange={handleInputChange} placeholder="Define your proprietary edge..." />
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2rem' }}>
                        ...
                        <button onClick={handleNext} className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', minWidth: '140px', justifyContent: 'center' }}>
                            {step === QUESTIONS.length - 1 ? (generating ? 'Blueprinting...' : 'Generate Blueprint') : 'Next Question'}
